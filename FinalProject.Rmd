...
final_set2 %>%
  ...
  labs(x = "Country", y = "KM",
       title = "Each Country of Africa's Forest Cover in Km")
...
final_set2 %>% filter(year==2020 | year==1995) %>% group_by(year) %>% 
  ...
  labs(title="Trend of percent forestry across the world in 1995 and 2020", x="average temperature", y="percent forestry")
```

Each dot represents a country in 1995 or 2020. From this plot, we can see that 
in both 1995 and 2020 (the earliest and latest recorded years in the dataset), 
there is a positive correlation between average temperature and percent of 
...
final_set2 %>% filter(round(average_temperature/10)*10==80) %>% 
  ...
  labs(title="Percent forestry at 75-84 degrees", x="percent forestry")
```
...
final_set2 %>% filter(year==1995 | year==2020) %>% group_by(country) %>% 
  ...
  geom_smooth(method=lm, se=FALSE) +
  labs(title="Forestry growth vs average temp (1995-2020)", x="average temperature", y="percent forestry growth")
```
